...
class Predict(nn.Module):
    def __init__(self, input_dim, model_dim, num_heads, ff_dim, dropout=0.1):
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=model_dim, nhead=num_heads, dim_feedforward=ff_dim, dropout=dropout
        )
...
class GateComDiffPretrainModel(nn.Module):
    def __init__(
        ...
        time_model=None,
        cross_mean=True,
        cross_model_real=None,
        cross_model_CrossSQLPlan=None,
        rootcause_cross_model=None,
        weight=0.5,
    ) -> None:
        super().__init__()
        ...
        for i in range(self.n_label):
            ...
            gate_sql_0.add_module(
                "gate_sql",
                nn.Linear(in_features=emb_dim, out_features=self.gate_out_dim),
            )
            ...
            gate_log_0.add_module(
                "gate_log", nn.Linear(in_features=emb_dim, out_features=emb_dim)
            )
            self.gate_log.append(gate_log_0)
            self.gate_log_activate.append(nn.Sigmoid())

            gate_metrics_0 = nn.Sequential()
            gate_metrics_0.add_module(
                "gate_metrics",
                nn.Linear(in_features=emb_dim, out_features=self.gate_out_dim),
            )
    ...
    def forward(self, sql, plan, time, log):
        ...
        common_emb = self.common_cross_model(
            sql_emb, plan_emb, log_emb, time_emb, None, None
        ).mean(dim=1)
        ...
        for i in range(self.n_label):
            ...
            emb = self.rootcause_cross_model(
                sql_emb_tmp, plan_emb_tmp, log_emb_tmp, time_emb_tmp, None, None
            )
