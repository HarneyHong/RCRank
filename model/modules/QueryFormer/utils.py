...
def formatJoin(json_node):
    ...
    if join is not None:
        ...
        twoCol = [
            (
                json_node["Alias"] + "." + col
                if len(col.split(".")) == 1 and "Alias" in json_node.keys()
                else col
            )
            for col in twoCol
        ]
...
class TreeNode:
    def __init__(
        ...
        self.start_up_cost, self.total_cost, self.plan_rows, self.plan_width = (
            start_up_cost,
            total_cost,
            plan_rows,
            plan_width,
    ...
    def __str__(self):
        return "{} with {}, {}, {} children".format(
            self.nodeType, self.filter, self.join_str, len(self.children)
        )
