...
class MarginLoss(nn.Module):
    ...
    def forward(self, pred, label):
        ...
        mask = torch.triu(torch.ones(label_num, label_num), diagonal=2) + torch.tril(
            torch.ones(label_num, label_num), diagonal=0
        )
...
class ThresholdLoss(nn.Module):
    ...
    def forward(self, pred, label):
        ...
        sign = ((label - self.threshold) + 1e-6) / torch.abs(
            (label - self.threshold) + 1e-6
        )
        sign = sign.detach()

        ts_loss = (0.5 - 0.5 * sign) * (pred - self.threshold + self.margin_left) + (
            0.5 + 0.5 * sign
        ) * (self.threshold - pred + self.margin_right)
